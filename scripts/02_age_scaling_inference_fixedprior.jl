...
for i in 1:n_iters
    for h in tree_heights#[7:end]
        ...
        ## save data
        fpath = string("output/simulations/age_scaling_effect_fixedprior/newick/h", Int64(h), "_", i ,".tre")
        writenewick(fpath, data, rates)

        fpath = string("output/simulations/age_scaling_effect_fixedprior/rates/h", Int64(h), "_", i ,".csv")
        CSV.write(fpath, rates)

        fpath = string("output/simulations/age_scaling_effect_fixedprior/jld2/h", Int64(h), "_", i ,".jld2")
        #Nsum = sum(N, dims = 1)[1,:,:]
        save(fpath, 
            #"N", N,
            #"Nsum", Nsum,
